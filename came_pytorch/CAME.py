...
class CAME(Optimizer):
    ...
    def _quantize_state(self, A, block_size):
        n_elements = A.numel()
        if n_elements <= 1:
            return A, {}
    ...
    def step_param(self, p, group):
        ...
        if len(state) == 0:
            ...
            if use_8bit:
                state["exp_avg"], state["exp_avg_quant_state"] = self._quantize_state(torch.zeros_like(grad), group["block_size"])
            else:
            ...
            if factored:
                ...
                if use_8bit:
                    state["exp_avg_sq"], state["exp_avg_sq_quant_state"] = self._quantize_state(torch.zeros_like(grad), group["block_size"])
                else:
        ...
        if use_8bit:
            exp_avg = self._dequantize_state(state["exp_avg"], state["exp_avg_quant_state"])
        else:
        ...
        if factored:
            ...
            if use_8bit:
                exp_avg_sq = self._dequantize_state(state["exp_avg_sq"], state["exp_avg_sq_quant_state"])
            else:
                exp_avg_sq = state["exp_avg_sq"]
            exp_avg_sq.mul_(group["betas"][1]).add_(update, alpha=1.0 - group["betas"][1])
            if use_8bit:
                state["exp_avg_sq"], state["exp_avg_sq_quant_state"] = self._quantize_state(exp_avg_sq, group["block_size"])
            else:
        ...
        if use_8bit:
            state["exp_avg"], state["exp_avg_quant_state"] = self._quantize_state(exp_avg, group["block_size"])
        else:
